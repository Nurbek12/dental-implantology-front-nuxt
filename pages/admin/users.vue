...
import type { Doctor, IDoctor, Specialty } from '@/types'
...
const save = async () => {
    try {
        ...
        Object.keys(doctor).map(key => formdata.append(key, doctor[key as keyof typeof doctor] as string))
        
        formdata.delete('id')

        if(file.value) formdata.append('image', file.value)

        if(itemIndex.value !== null) await update(doctor.id!, itemIndex.value, formdata)
